...
from spectral_monitoring import (
    spectral_monitoring_epoch,
    adaptive_mixture_adjustment,
...
# Spectral monitoring parameters
parser.add_argument('--spectral_monitor', action='store_true', help='Enable spectral monitoring')
parser.add_argument('--spectral_freq', type=int, default=10, help='Frequency of spectral monitoring (epochs)')
parser.add_argument('--spectral_adaptive', action='store_true', help='Enable adaptive mixture adjustment')
parser.add_argument('--spectral_reg', action='store_true', help='Enable spectral regularization loss')
parser.add_argument('--spectral_reg_alpha', type=float, default=0.01, help='Weight for spectral regularization')
parser.add_argument('--spectral_k_neighbors', type=int, default=50, help='Number of neighbors for spectral graph')
parser.add_argument('--spectral_analysis', action='store_true', help='Analyze spectral impact of wild data')
...
# spectral monitoring metrics
state['spectral_gaps'] = []
state['spectral_degradation'] = []

def to_np(x): return x.data.cpu().numpy()

def analyze_pi_impact(epoch, net, train_loader_in, train_loader_aux_in,
                      train_loader_aux_in_cor, train_loader_aux_out, args):
    """Quick analysis of how different pi values affect spectral gap"""
    if not args.spectral_analysis or epoch % 50 != 0:
        return

    print(f"\n[Spectral Analysis] Epoch {epoch}")

    # Test different pi combinations
    test_configs = [
    ...
    for pi_1, pi_2 in test_configs:
        # Temporarily set pi values
        orig_pi_1, orig_pi_2 = args.pi_1, args.pi_2
        args.pi_1, args.pi_2 = pi_1, pi_2

        # Get one batch with this mixture
        in_batch = next(iter(train_loader_in))
        aux_in_batch = next(iter(train_loader_aux_in))
        aux_cor_batch = next(iter(train_loader_aux_in_cor))
        aux_out_batch = next(iter(train_loader_aux_out))

        # Mix according to pi values
        mixed_batch = mix_batches(aux_in_batch, aux_cor_batch, aux_out_batch)
        ...
        gap_in = compute_spectral_gap(features_in, k=50)
        gap_mixed = compute_spectral_gap(
            torch.cat([features_in, features_wild]), k=50
        )
        ...
        results.append({
            ...
            'gap_in': gap_in,
            'gap_mixed': gap_mixed,
            'degradation': 1 - gap_mixed/gap_in if gap_in > 0 else 0
        })

        # Restore original pi values
        args.pi_1, args.pi_2 = orig_pi_1, orig_pi_2

    # Print results
    print("\nπ₁(cov)  π₂(label)  Gap(in)  Gap(mixed)  Degradation")
    print("-" * 55)
    for r in results:
        print(f"{r['pi_1']:.1f}      {r['pi_2']:.1f}        "
              f"{r['gap_in']:.4f}   {r['gap_mixed']:.4f}     "
              f"{r['degradation']:+.1%}")

    # Find best configuration
    best = min(results, key=lambda x: x['degradation'])
    print(f"\nBest configuration: π₁={best['pi_1']}, π₂={best['pi_2']}")
...
def train(epoch):
    ...
    for in_set, aux_in_set, aux_in_cor_set, aux_out_set in loaders:
        ...
        # Extract features for spectral regularization if enabled
        features_in = None
        features_aux = None
        ...
        # Add spectral regularization if enabled
        if args.spectral_reg and features_in is not None:
            loss_spectral = spectral_regularization_loss(
                features_in, k=args.spectral_k_neighbors, alpha=args.spectral_reg_alpha
            )
            loss = loss + loss_spectral

        # Additional spectral regularization for woods_nn and scone
        if args.score in ['woods_nn', 'scone'] and epoch >= args.start_epoch and args.spectral_reg:
            # Extract features for spectral regularization if not already done
            ...
            gap = compute_spectral_gap(features_in, k=50)
            device = features_in.device if torch.is_tensor(features_in) else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            loss_spectral_extra = -args.spectral_reg_alpha * torch.log(torch.tensor(gap + 1e-6).to(device))

            # Add to total loss
            loss = loss + loss_spectral_extra

            # Log it
            if batch_num % args.print_freq == 0:
                print(f"  Spectral gap: {gap:.4f}, Loss: {loss_spectral_extra.item():.4f}")
    ...
    # Spectral monitoring
    if args.spectral_monitor and epoch % args.spectral_freq == 0:
        spectral_stats = spectral_monitoring_epoch(
            net, train_loader_in, train_loader_aux_in,
            train_loader_aux_in_cor, train_loader_aux_out,
            args, epoch, sample_batches=3
        )

        # Store spectral statistics
        state['spectral_gaps'].append(spectral_stats)
        state['spectral_degradation'].append(spectral_stats.get('degradation', 0))

        # Optional: adaptive adjustment
        if args.spectral_adaptive and epoch >= args.start_epoch:
            args.pi_1, args.pi_2 = adaptive_mixture_adjustment(spectral_stats, args)

    # Spectral analysis of pi impact
    if args.spectral_analysis:
        spectral_results = analyze_pi_impact(epoch, net, train_loader_in,
                                            train_loader_aux_in, train_loader_aux_in_cor,
                                            train_loader_aux_out, args)
